...
class GalleryViewWidget(QFrame):
    ...
    def __init__(self, rows = 10, columns = 10, dataHandler=None, dataHandler2=None, is_main_gallery=False, is_for_similarity_search=False, parent=None, *args, **kwargs):
        ...
        self.btn_show_on_UMAP_live.setChecked(False)
...
class MainWindow(QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        # process a FOV
        I_fluorescence = imageio.v2.imread(FILENAME_prefix + '_0_Fluorescence_405_nm_Ex.bmp')
        I_fluorescence = I_fluorescence[:,:,::-1]
        I_BF_left = imageio.v2.imread(FILENAME_prefix + '_0_BF_LED_matrix_left_half.bmp')
        I_BF_right = imageio.v2.imread(FILENAME_prefix + '_0_BF_LED_matrix_right_half.bmp')
        spot_images, I_DPC = process_fov(I_fluorescence,I_BF_left,I_BF_right)
        # np.save('data.npy',I*255)

        segmentation_model_path = 'models/model_70_11.pth'
        segmentation_model = m2u(pretrained_model=segmentation_model_path, use_trt=False)

        # run segmentation
        n_cells, mask, mask_overlay = run_segmentation(I_DPC,segmentation_model,save_overlay=True)
        imageio.v2.imwrite(FILENAME_prefix + '_mask.bmp',mask)
        imageio.v2.imwrite(FILENAME_prefix + '_mask_overlay.bmp',mask_overlay)

        # display the result
        self.dataHandler.load_model('models/model_perf_r34_b32.pt')
        # self.dataHandler.load_images('data.npy')
        self.dataHandler.load_images_direct((spot_images*255).astype('uint8'))
