...
def process_fov(I_fluorescence,I_BF_left,I_BF_right):
    ...
    spot_images = get_spot_images_from_fov(I_fluorescence,I_DPC,spot_list,r=15)
    spot_images = spot_images.transpose(0, 3, 1, 2)

    return spot_images, (I_DPC * 255).astype(np.uint8)
...
def run_segmentation(I_DPC,segmentation_model,save_overlay):

    result = segmentation_model.predict_on_images(I_DPC)
    threshold = 0.5
    mask = (255*(result > threshold)).astype(np.uint8)
    # mask_path = FILENAME_prefix + '_mask.bmp'
    # with dest_open(mask_path, 'wb') as f:
    #     imageio.imwrite(f.name, mask, format='bmp')

    labeled_mask, n_cells = label(mask)
    print('------------------------')
    print( str(n_cells) + ' red blood cells' )
    print('------------------------')

    mask_overlay = overlay_mask_dpc(colorize_mask(labeled_mask), I_DPC)

    # # Store overlay
    # if save_overlay: 
        
    #     overlay_path = FILENAME_prefix + '_mask_overlay.bmp'
    #     with dest_open(overlay_path, 'wb') as f:
    #         imageio.imwrite(f.name, overlay, format='bmp')

    return n_cells, mask, mask_overlay