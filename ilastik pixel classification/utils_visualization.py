...
def process_fov_for_spot_visualizations(I_fluorescence,I_dpc,spot_data,parameters,dir_out,r=30,scale=5,image_format='png'):
	# make I_dpc RGB
...
def save_cropped_spot(I_overlay,hsv,mask,i,j,x,y,dir_out,r=30,scale=5,image_format='png'):
	# get the full image size
	height,width,channels = I_overlay.shape
	# create the arrays for cropped images
	I_overlay_cropped = np.zeros((2*r+1,2*r+1,3), np.float)
	I_mask_cropped = np.zeros((2*r+1,2*r+1,3), np.float)
	I_hsv_cropped = np.zeros((2*r+1,2*r+1,3), np.float)
	# identify cropping region in the full FOV 
	x_start = max(0,x-r)
	x_end = min(x+r,width-1)
	y_start = max(0,y-r)
	...
	# do the cropping 
	I_overlay_cropped[y_idx_cropped,x_idx_cropped,:] = I_overlay[y_idx_FOV,x_idx_FOV,:]
	I_mask_cropped[y_idx_cropped,x_idx_cropped,:] = mask[y_idx_FOV,x_idx_FOV,:]
	I_hsv_cropped[y_idx_cropped,x_idx_cropped,:] = hsv[y_idx_FOV,x_idx_FOV,:]
	# put the images in a row
	row = np.hstack((I_overlay_cropped,I_hsv_cropped,I_mask_cropped))
	row = cv2.resize(row,None,fx=scale,fy=scale,interpolation=cv2.INTER_NEAREST)
	cv2.imwrite(dir_out + '/' + str(i) + '_' + str(j) + '_' + str(x) + '_' + str(y) + '.' + image_format, row)