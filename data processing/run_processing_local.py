...
if __name__ == '__main__':
  ...
  bucket_source = '/media/hongquan/Extreme SSD/octopi 2023'
  ...
  settings['spot_detection_threshold'] = 8
  ...
  for dataset_id in DATASET_ID:
    ...
    os.makedirs('./output/' + dataset_id + '/spot_detection_result',exist_ok=True)
    os.makedirs('./output/' + dataset_id + '/spot_lists',exist_ok=True)
    os.makedirs('./output/' + dataset_id + '/spot_data',exist_ok=True)

    # processing
    print('processing ' + dataset_id + ' in ' + bucket_source)
    with get_context("spawn").Pool(processes=8) as pool:
      pool.map(partial(process_column,gcs_settings=gcs_settings,dataset_id=dataset_id,parameters=parameters,settings=settings,fs=fs),columns)
    ...
    if pooling_and_plotting:
      ...
      with fs.open( bucket_destination + '/' + dataset_id + '/' + 'scatter plot_raw.png', 'wb' ) as f:
        plt.savefig(f)
      with fs.open( bucket_destination + '/' + dataset_id + '_' + 'scatter plot_raw.png', 'wb' ) as f:
        plt.savefig(f)
      print(R.size)
