...
data_dir_n = '/media/rinni/Extreme SSD/Rinni/Octopi/data/neg-to-combine/'
...
def isolate_wrong_predictions(ann_dict, class_key, in_im_path, in_ann_w_pred_path, out_dir = None):
	...
	np.save(out_dir + '/combined_images_' + class_key + '_wrong_r18_b32.npy', images_w)
	ann_df_w.to_csv(out_dir + '/combined_ann_' + class_key + '_wrong_r18_b32.csv')	

# train model given input annotations and images; output performance??
def model_training(ann_dict, in_im_path, in_ann_path, out_ann_w_pred_path, out_model_path, model_specs, train_frac=0.7, n_epochs=25):
	# model input prep!
...
def train_model(model, images, annotations, out_model_path, train_frac=0.7, batch_size=32, n_epochs=40):
	...
	np.savetxt(out_model_path[:out_model_path.rfind('/')] + '/indices_r18_b32.csv', indices[int(len(images)*0.7):], delimiter=',')
	data = images[indices,:,:,:]
	label = annotations[indices]

	# make images 0-1 if they are not already
	if data.dtype == np.uint8:
		print(sys.getsizeof(data)/(1024 ** 3))
	...
	loss_df.to_csv(out_model_path[:out_model_path.rfind('/')] + '/loss_df_r18_b32.csv')
	# training complete
...
def relabel_uns_by_diff_predictions(in_ann_w_pred_path, class_val, class_name = 'unsure', out_ann_path = None, relabel_thresh = 0.9, class_to_relabel_by = 'parasite'):
	...
	if out_ann_path is None: # default path
		diff_ann_relabeled_path = '/'.join(in_ann_w_pred_path.split('/')[:-1]) + '/' + class_name + '_r18_b32_relabeled_thresh_' + "{:.1f}".format(relabel_thresh) + '.csv'
...
def differentiator_classifier_wrapper(strat_name, uns_label, combine_dict_diff, combine_dict_cl, model_spec = model_spec1):
	...
	diff_images_path = data_dir_base + '/' + strat_name + '/combined_images_diff_r18_b32.npy'
	diff_ann_path = data_dir_base + '/' + strat_name + '/combined_ann_diff_r18_b32.csv'
	combine_datasets(combine_dict_diff, diff_images_path, diff_ann_path)

	# train differentiator 1
	diff_ann_w_pred_path = data_dir_base + '/' + strat_name + '/ann_with_predictions_diff_r18_b32.csv'
	diff_model_perf_path = data_dir_base + '/' + strat_name + '/model_perf_diff_r18_b32.pt'
	...
	cl_images_path = data_dir_base + '/' + strat_name + '/combined_images_cl_r18_b32.npy'
	cl_ann_path = data_dir_base + '/' + strat_name + '/combined_ann_cl_r18_b32.csv'
	combine_datasets(combine_dict_cl, cl_images_path, cl_ann_path)

	# train classifier 1
	cl_ann_w_pred_path = data_dir_base + '/' + strat_name + '/ann_with_predictions_cl_r18_b32.csv'
	cl_model_perf_path = data_dir_base + '/' + strat_name + '/model_perf_cl_r18_b32.pt'
...
# combine_datasets(slide_images_dict, data_dir + '/combined_images_all.npy', data_dir + '/combined_ann_all.csv')
...
cl_sa_ann_w_pred_path = data_dir_base + '/s_a/ann_with_predictions_r18_b32.csv'
cl_sa_model_path = data_dir_base + '/s_a/model_perf_r18_b32.pt'
# model_training(ann_dict, combined_image_path, combined_ann_path, cl_sa_ann_w_pred_path, cl_sa_model_path, model_spec1) #TODO: PUT BACK!

# SB: classifier run on all pos/neg images (not unsure)
cl_sb_ann_w_pred_path = data_dir_base + '/s_b/ann_with_predictions_r18_b32.csv'
cl_sb_model_path = data_dir_base + '/s_b/model_perf_r18_b32.pt'
...
# combine_dict_diff[data_dir_base + '/s_a/combined_images_non-parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_a/combined_ann_non-parasite_wrong_r18_b32.csv'
...
# combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_1a/unsure_r18_b32_relabeled_thresh_0.9.csv'
...
# combine_dict_diff = {}
# combine_dict_diff[data_dir + '/combined_images_unsure.npy'] = data_dir + '/combined_ann_unsure_as_parasite.csv'
# combine_dict_diff[data_dir_base + '/s_b/combined_images_non-parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_b/combined_ann_non-parasite_wrong_r18_b32.csv'

# # datasets to combine for classifier 1b: note that the relabeling hasn't actually happened yet!
# combine_dict_cl = {}
# combine_dict_cl[data_dir + '/combined_images_parasite_and_non-parasite.npy'] = data_dir + '/combined_ann_parasite_and_non-parasite.csv'
# combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_1b/unsure_r18_b32_relabeled_thresh_0.9.csv'
...
# combine_dict_diff[data_dir + '/combined_images_unsure.npy'] = data_dir + '/combined_ann_unsure_as_parasite.csv'
# combine_dict_diff[data_dir_base + '/s_a/combined_images_non-parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_a/combined_ann_non-parasite_wrong_r18_b32.csv'
# combine_dict_diff[data_dir_base + '/s_a/combined_images_parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_a/combined_ann_parasite_wrong_r18_b32.csv'

# # datasets to combine for classifier 2a: note that the relabeling hasn't actually happened yet!
# combine_dict_cl = {}
# combine_dict_cl[data_dir + '/combined_images_parasite_and_non-parasite.npy'] = data_dir + '/combined_ann_parasite_and_non-parasite.csv'
# combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_2a/unsure_r18_b32_relabeled_thresh_0.9.csv'
...
# combine_dict_diff[data_dir + '/combined_images_unsure.npy'] = data_dir + '/combined_ann_unsure_as_parasite.csv'
# combine_dict_diff[data_dir_base + '/s_b/combined_images_non-parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_b/combined_ann_non-parasite_wrong_r18_b32.csv'
# combine_dict_diff[data_dir_base + '/s_b/combined_images_parasite_wrong_r18_b32.npy'] = data_dir_base + '/s_b/combined_ann_parasite_wrong_r18_b32.csv'

# # datasets to combine for classifier 2b: note that the relabeling hasn't actually happened yet!
# combine_dict_cl = {}
# combine_dict_cl[data_dir + '/combined_images_parasite_and_non-parasite.npy'] = data_dir + '/combined_ann_parasite_and_non-parasite.csv'
# combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_2b/unsure_r18_b32_relabeled_thresh_0.9.csv'
...
combine_dict_cl = {}
combine_dict_cl[data_dir + '/combined_images_parasite_and_non-parasite.npy'] = data_dir + '/combined_ann_parasite_and_non-parasite.csv'
combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_3a/unsure_r18_b32_relabeled_thresh_0.9.csv'
...
combine_dict_cl = {}
combine_dict_cl[data_dir + '/combined_images_parasite_and_non-parasite.npy'] = data_dir + '/combined_ann_parasite_and_non-parasite.csv'
combine_dict_cl[data_dir + '/combined_images_unsure.npy'] = data_dir_base + '/s_3b/unsure_r18_b32_relabeled_thresh_0.9.csv'
