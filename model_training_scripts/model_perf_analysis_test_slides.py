...
files_analyzed = []
fp_dict = {}
# Loop through all annotations with predictions files in the directory
for filename in os.listdir(data_dir):
    # Check if file name contains the model architecture
    if model_arch in filename and 'ann' in filename:
        files_analyzed.append(filename)
        
        if unsure_ignored:
            performance_out_path = data_dir + filename.split("_ann")[0] + '_model' + model_arch + '_performance_' + pos_class + '_v_rest_unsure_ignored.csv'
        else:
            performance_out_path = data_dir + filename.split("_ann")[0] + '_model' + model_arch + '_performance_' + pos_class + '_v_rest.csv'
        ann_w_pred_df = pd.read_csv(data_dir + filename,index_col='index')
        ...
        perf_df.loc[mcc_mask,'MCC'] = perf_df.loc[mcc_mask,'MCC'] / (perf_df.loc[mcc_mask,'predicted pos']*perf_df.loc[mcc_mask,'predicted neg']*(pos_num)*(neg_num)).fillna(1).apply(np.sqrt)
